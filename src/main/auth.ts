...
const auth0Domain = 'devinai.us.auth0.com'
...
const payUrl = `${HOMEPAGE_ROOT}/pricing?fromDevinai=true`
...
export function authPackage() {
    ...
    ipcMain.handle('loginDevinai', login)
    ipcMain.handle('signupDevinai', signup)
    ipcMain.handle('payDevinai', pay)
    ipcMain.handle('settingsDevinai', settings)
    ipcMain.handle('logoutDevinai', createLogoutWindow)
