...
import { editBoundaryEffect, insertDevinaiEffect } from '../extensions/hackDiff'
...
const getBearerTokenHeader = (getState: () => unknown) => {
    const accessToken = (getState() as FullState).toolState.devinaiLogin
        .accessToken
    if (accessToken) {
        return {
...
export async function getPayload({
    ...
    const userRequest = {
        ...
        currentFileContents,
        // Context surrounding the devinai position
...
export const diffResponse = createAsyncThunk(
    ...
    async (type: 'lsp' | 'chat' | null, { getState, dispatch }) => {
        try {
            ...
            // If the devinai is in a current pos, set the active line to the top of that
        ...
        switch (keypress) {
            ...
            case 'k':
                ...
                if (editorView) {
                    ...
                    const devinaiPos = selection.from

                    //const preceedingPos = getPrecedingLinesPos(view, 20);
                    editorView.dispatch({
                        effects: insertDevinaiEffect.of({
