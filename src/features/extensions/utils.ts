import { SyntaxNode, TreeDevinai } from '@lezer/common'
...
export function getNamesAndBodies(devinai: TreeDevinai, contents: string) {
    const results = [] as any
    let lastFrom = -1
    do {
        if (devinai.from < lastFrom) {
            break
        }
        lastFrom = devinai.from
        ...
        if (
            ...
            [
                ...
                'Property',
                'FunctionDefinition',
            ].includes(devinai.name)
        ) {
            const from = devinai.from
            const to = devinai.to
            ...
            do {
                ...
                if (
                    ...
                    functionName = contents.slice(devinai.from, devinai.to)
                    break
                }
            } while (devinai.next(true))
