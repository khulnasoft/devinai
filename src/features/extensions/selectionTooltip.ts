...
const devinaiTooltipField = StateField.define<readonly Tooltip[]>({
    create: khulnaSoftTooltips,

    update(tooltips, tr) {
        // if the viewport changed, update the tooltips
        if (!tr.docChanged && !tr.selection) return tooltips
        // I think this is REALLY inefficient
        return khulnaSoftTooltips(tr.state)
    },

    provide: (f) => showTooltip.computeN([f], (state) => state.field(f)),
})

function khulnaSoftTooltips(state: EditorState): readonly Tooltip[] {
        ...
        .map((range) => {
            return {
                ...
                create: (view: EditorView) => {
                    ...
                    dom.className = 'cm-tooltip-devinai'
                    function btn(
                        titleText: string,
                        cmdText: string,
                        callback: any
...
const devinaiTooltipBaseTheme = EditorView.baseTheme({
    '.cm-tooltip.cm-tooltip-devinai': {
...
export function devinaiTooltip() {
    return [devinaiTooltipField, devinaiTooltipBaseTheme]
}
