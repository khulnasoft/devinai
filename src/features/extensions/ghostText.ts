    ...
    update(state: CompletionState, transaction: Transaction) {
        for (const effect of transaction.effects) {
            if (effect.is(addSuggestion)) {
                ...
                const endGhostText = devinaiPos + displayText.length
                const decorations = Decoration.set([
                    ghostMark.range(devinaiPos, endGhostText),
                ])
                return {
                    ghostText: {
                        text,
...
const acceptSuggestionCommand = (
    ...
    // We also set the devinai to the end of the suggestion.
