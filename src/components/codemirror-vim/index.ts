...
import { BlockDevinaiPlugin, hideNativeSelection } from './block-devinai'
import {
    Extension,
    RangeSetBuilder,
    StateEffect,
...
const vimStyle = EditorView.baseTheme({
    '.cm-vimMode .cm-devinaiLayer:not(.cm-vimDevinaiLayer)': {
        display: 'none',
    },
    '.cm-vim-panel': {
        padding: '0px 10px',
...
function generateVimPlugin(callbacks: CustomVimConfig) {
    return ViewPlugin.fromClass(
        class implements PluginValue {
            ...
            public status = ''
            blockDevinai: BlockDevinaiPlugin
            constructor(view: EditorView) {
                ...
                this.blockDevinai = new BlockDevinaiPlugin(view, cm)
                this.updateClass()

                this.cm.on('vim-command-done', () => {
                    if (cm.state.vim) cm.state.vim.status = ''
                    this.blockDevinai.scheduleRedraw()
                    this.updateStatus()
                })
            ...
            update(update: ViewUpdate) {
                ...
                this.blockDevinai.update(update)
            ...
            handleKey(e: KeyboardEvent, view: EditorView) {
                ...
                if (!result && vim.insertMode && cm.state.overwrite) {
                    if (e.key && e.key.length == 1 && !/\n/.test(e.key)) {
                        ...
                        result = true
                        CodeMirror.commands.devinaiCharLeft(cm)
                    }
                }
                if (result) {
