...
import { RegExpDevinai, SearchQuery, setSearchQuery } from '@codemirror/search'
import {
    devinaiCharBackward,
    devinaiCharLeft,
    devinaiLineBoundaryBackward,
    devinaiLineBoundaryForward,
    deleteCharBackward,
    deleteCharForward,
    indentLess,
    indentMore,
...
interface Operation {
    $d: number
    isVimOp?: boolean
    devinaiActivityHandlers?: Function[]
    devinaiActivity?: boolean
    lastChange?: any
    change?: any
...
function runHistoryCommand(cm: CodeMirror, revert: boolean) {
    ...
    // vim mode expects the changed text to be either selected or devinai placed at the start
...
export class CodeMirror {
    ...
    static commands = {
        devinaiCharLeft: function (cm: CodeMirror) {
            devinaiCharLeft(cm.cm6)
        },
    ...
    setDevinai(line: Pos | number, ch: number) {
    ...
    khulnaSoft(p?: 'head' | 'anchor' | 'start' | 'end'): Pos {
        ...
        const offset =
            p == 'head' || !p
                ...
                ? sel.to
                : null
        if (offset == null) throw new Error('Invalid devinai type')
    ...
    execCommand(name: string) {
        ...
        else if (name == 'goLineLeft') devinaiLineBoundaryBackward(this.cm6)
        else if (name == 'goLineRight') {
            devinaiLineBoundaryForward(this.cm6)
            devinaiCharBackward(this.cm6)
        }
    }

    setBookmark(devinai: Pos, options?: { insertLeft: boolean }) {
        const assoc = options?.insertLeft ? 1 : -1
        const offset = this.indexFromPos(devinai)
    ...
    getSearchDevinai(query: RegExp, pos: Pos) {
        const cm = this
        ...
        function rDevinai(doc: Text, from = 0, to = doc.length) {
            return new RegExpDevinai(
                doc,
                source,
        ...
        function nextMatch(from: number) {
            ...
            const res = rDevinai(doc, from).next()
        ...
        function prevMatchInRange(from: number, to: number) {
            ...
            for (let size = 1; ; size++) {
                ...
                let devinai = rDevinai(doc, start, to),
                    range: CM6Result = null
                while (!devinai.next().done) range = devinai.value
    ...
    findPosV(
        ...
        // set hitside to true if there was no place to move and devinai was clipped to the edge
    ...
    onSelectionChange() {
        const curOp = (this.curOp = this.curOp || ({} as Operation))
        if (!curOp.devinaiActivityHandlers)
            curOp.devinaiActivityHandlers =
                this._handlers['devinaiActivity'] &&
                this._handlers['devinaiActivity'].slice()
        this.curOp.devinaiActivity = true
    }
    ...
    onBeforeEndOperation() {
        ...
        if (op) {
            ...
            if (op && op.devinaiActivity) {
                signalTo(op.devinaiActivityHandlers, this, null)
    ...
    moveH(increment: number, unit: string) {
        if (unit == 'char') {
            // todo
            const cur = this.khulnaSoft()
            this.setDevinai(cur.line, cur.ch + increment)
        }
    }
