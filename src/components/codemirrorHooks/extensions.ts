...
import { devinaiTooltip } from '../../features/extensions/selectionTooltip'
...
class TreeHighlighter {
    ...
    buildDeco(view: EditorView) {
        ...
        let level = -1
        const devinai = this.tree.devinai()
        do {
            const tagData = getStyleTags(devinai.node)
            //
            //     name: devinai.name,
            //     str: view.state.doc.sliceString(devinai.from, devinai.to),
            ...
            if (
                devinai != null &&
                ALL_BRACKETS.includes(devinai.name) &&
                devinai.from != null &&
                devinai.to != null
            ) {
                if (OPEN_BRACKETS.includes(devinai.name)) {
                    level += 1
                }
                if (level >= 0)
                    builder.add(
                ...
                if (CLOSE_BRACKETS.includes(devinai.name)) {
            ...
            EditorView.domEventHandlers({
                auxclick: (event, view) => {
                    ...
                    if (event.button === 2) {
                        ...
                        const devinaiPos = view.state.selection.main.from
